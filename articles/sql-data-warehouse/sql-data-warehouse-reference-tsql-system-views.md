<properties
   ...
   description="Des liens vers le système affiche le contenu de l’entrepôt de données SQL."
...
## <a name="sql-server-dmvs-applicable-to-sql-data-warehouse"></a>DMV de SQL Server applicable à l’entrepôt de données SQL

Les DMV suivantes sont applicables à l’entrepôt de données SQL, mais doivent être exécutés par la connexion à la base de données **master** .
...
## <a name="sql-server-dmvs-available-in-sql-data-warehouse"></a>DMV SQL Server disponible dans l’entrepôt de données SQL

SQL Data Warehouse présente quasiment les vues de gestion dynamique (DMV) de SQL Server. Ces vues, lorsqu’il est interrogé dans l’entrepôt de données SQL, signalent l’état des bases de données SQL en cours d’exécution sur les distributions.

Dans la mesure où SQL Data Warehouse est basé sur la technologie MPP de Microsoft, SQL Data Warehouse et entrepôt de données parallèle (PDW du système Analytique plate-forme) utilisent les mêmes vues système.

C’est pourquoi chacun de ces DMV a une colonne appelée pdw_node_id. Il s’agit de l’identificateur pour le nœud de calcul. Dans l’Assistant Empaquetage et déploiement du calcul le nœud est un concept plus puissant de l’architecture. SQL Data Warehouse, l’architecture appuie plus sur les distributions.

>[AZURE.NOTE] Pour utiliser ces affichage, insérez 'pdw_nodes_' dans le nom, comme indiqué dans le tableau suivant.


| Nom de vue dans le magasin de données SQL | Créer un lien vers la rubrique Transact-SQL de SQL Server sur MSDN |
| :----------------------------- | :-------------------------------------------- |
...
## <a name="sql-server-2016-polybase-dmvs-available-in-sql-data-warehouse"></a>PolyBase de DMV SQL Server 2016 disponible dans l’entrepôt de données SQL
...
## <a name="sql-server-informationschema-views"></a>Vues INFORMATION_SCHEMA de SQL Server
