<properties
   pageTitle="Exemple de cycle de vie d’application basée sur le reste | Microsoft Azure"
   description="Exemple Microsoft Azure Fabric de Service qui présente le cycle de vie de l’application à l’aide de l’interface REST de tissu de Service."
...
# <a name="rest-based-application-lifecycle-sample"></a>Exemple de cycle de vie d’application basée sur le reste

Cet exemple illustre le cycle de vie de l’application Fabric de Service via des appels de l’API REST. Pour plus d’informations sur le cycle de vie des applications Fabric de Service, consultez [cycle de vie d’application Fabric du Service](service-fabric-application-lifecycle.md).

Cet exemple effectue les tâches suivantes :

* Dispositions de stocker de l’exemple **WordCount 1.0.0** à partir du package d’application WordCount dans l’image.
* Affiche la liste des types d’application, qui inclut les WordCount 1.0.0.
* Crée l’application WordCount comme **fabric : / WordCount**.
* Affiche la liste des applications, ce qui inclut le fabric : / WordCount version 1.0.0.
* Version de dispositions le 1.1.0 de l’exemple WordCount à partir du package d’application **WordCountUpgrade** dans le magasin d’image.
* Affiche la liste des types d’application, qui inclut à la fois WordCount 1.0.0 et **WordCount 1.1.0**.
* Met à niveau l’application WordCount version 1.1.0.
* Affiche la liste des applications, ce qui inclut les WordCount version 1.1.0, mais n’inclut plus WordCount version 1.0.0.
* Supprime l’application WordCount.
* Affiche la liste des applications, qui ne contient plus le fabric : / WordCount.
* Version de unprovisions la 1.1.0 de l’exemple WordCount.
* Affiche la liste des types d’application, qui inclut les WordCount 1.0.0, mais n’inclut plus WordCount 1.1.0.
* Version de unprovisions la 1.0.0 de l’exemple WordCount.
* Affiche la liste des types d’application, qui n’inclut plus WordCount.


## <a name="prerequisites"></a>Conditions préalables

Cet exemple utilise l' [exemple WordCount](http://aka.ms/servicefabricsamples) (trouvée dans les exemples de **Mise en route** ). L’exemple WordCount doit être généré en premier, puis les deux packages d’application doivent être copiés dans le magasin d’image.

|Dossier|Description|
|------|-----------|
|WordCount|L’exemple d’application WordCount. Le fichier **ApplicationManifest.xml** contient **ApplicationTypeVersion = « 1.0.0 »**.|
|WordCountUpgrade|L’exemple d’application WordCount. Le fichier ApplicationManifest.xml doit être modifié en **ApplicationTypeVersion = « 1.1.0 »** afin de permettre la mise à niveau de l’application à se produire.|

Pour créer les packages d’application et les copier dans le magasin d’image, procédez comme suit :

1. Copiez **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** dans **C:\Temp\WordCount**. Cette opération crée le package d’application WordCount.
2. Copiez C:\Temp\WordCount dans **C:\Temp\WordCountUpgrade**. Cette opération crée le package **d’application de WordCountUpgrade** .
3. Ouvrez **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** dans un éditeur de texte.
4. Dans l’élément **ApplicationManifest** , modifier l’attribut **ApplicationTypeVersion** à **« 1.1.0 »**.  Cela met à jour le numéro de version de l’application.
5. Enregistrez le fichier modifié de la ApplicationManifest.xml.
6. Exécutez le script PowerShell suivant en tant qu’administrateur pour copier les applications vers le magasin d’image :
...
Une fois le script PowerShell, cette application est prête à s’exécuter.

## <a name="example"></a>Exemple

L’exemple suivant illustre le cycle de vie des applications Fabric de Service.
