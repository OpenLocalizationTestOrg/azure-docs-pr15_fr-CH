<properties
   pageTitle="Déployer des machines virtuelles de carte d’interface réseau multiples à l’aide de la CLI d’Azure dans le Gestionnaire de ressources | Microsoft Azure"
   description="Apprenez à déployer des ordinateurs virtuels de carte d’interface réseau multiples à l’aide de la CLI d’Azure dans le Gestionnaire de ressources"
...
#<a name="deploy-multi-nic-vms-using-the-azure-cli"></a>Déployer des machines virtuelles de carte d’interface réseau multiples à l’aide de la CLI d’Azure

[AZURE.INCLUDE [virtual-network-deploy-multinic-arm-selectors-include.md](../../includes/virtual-network-deploy-multinic-arm-selectors-include.md)]

[AZURE.INCLUDE [virtual-network-deploy-multinic-intro-include.md](../../includes/virtual-network-deploy-multinic-intro-include.md)]

[AZURE.INCLUDE [azure-arm-classic-important-include](../../includes/learn-about-deployment-models-rm-include.md)][modèle de déploiement classique](virtual-network-deploy-multinic-classic-cli.md).

[AZURE.INCLUDE [virtual-network-deploy-multinic-scenario-include.md](../../includes/virtual-network-deploy-multinic-scenario-include.md)]

Actuellement, vous ne peut pas avoir des ordinateurs virtuels avec une seule carte réseau et les ordinateurs virtuels avec plusieurs cartes réseau dans le même groupe de ressources. Par conséquent, vous devez mettre en œuvre les serveurs principaux dans un groupe de ressources différent de tous les autres composants. Les étapes ci-dessous utilisent un groupe de ressource nommé *IaaSStory* pour le groupe de ressources principal et *IaaSStory-back-end* pour les serveurs principaux.
...
Vous pouvez télécharger le script bash complet utilisé [ici](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/11-MultiNIC/arm/virtual-network-deploy-multinic-arm-cli.sh). Suivez les étapes ci-dessous pour modifier le script fonctionne dans votre environnement.
...
3. Récupérer l’ID de la `BackEnd` sous-réseau où seront créés les ordinateurs virtuels. Vous devez pour ce faire, dans la mesure où les cartes à associer à ce sous-réseau se trouvent dans un groupe de ressources différent.
...
4. Récupérer l’ID de la `NSG-RemoteAccess` NSG. Vous devez pour ce faire, dans la mesure où les cartes à associer à cette NSG se trouvent dans un groupe de ressources différent.
...
1. Créer un nouveau groupe de ressources pour toutes les ressources de serveur principal. Notez l’utilisation de la `$backendRGName` variable pour le nom de groupe de ressources, et `$location` de la région d’Azure.
...
3. Créer une disponibilité pour les ordinateurs virtuels.
...
2. Pour chaque machine virtuelle, créer une carte d’interface réseau pour l’accès de la base de données.
...
3. Pour chaque machine virtuelle, créer une carte d’interface réseau pour l’accès distant. Avis du `--network-security-group` paramètre, utilisé pour associer la carte réseau à un NSG.
...
5. Pour chaque machine virtuelle, créer deux disques de données et mettre fin à la boucle avec le `done` commande.
