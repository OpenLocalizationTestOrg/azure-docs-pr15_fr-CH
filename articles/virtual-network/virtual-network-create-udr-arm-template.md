<properties 
   pageTitle="Contrôler le routage et utiliser des boîtiers virtuels dans le Gestionnaire de ressources en utilisant un modèle | Microsoft Azure"
   description="Apprenez à contrôler le routage et utiliser des boîtiers virtuels dans le Gestionnaire de ressources Azure à l’aide d’un modèle"
...
#<a name="create-user-defined-routes-udr-in-resource-manager-by-using-a-template"></a>Créer des itinéraires de défini par l’utilisateur (UDR) dans le Gestionnaire de ressources à l’aide d’un modèle
...
La section ci-dessous illustre la définition de la partie frontale UDR dans le fichier **azuredeploy-vnet-nsg-udr.json** , en fonction du scénario ci-dessus.
...
Pour associer le UDR au sous-réseau front-end, vous devez modifier la définition de sous-réseau dans le modèle et utilisez l’id de référence pour l’UDR.
...
Vous devez également vous assurer que la machine virtuelle de **FW1** a transfert de propriété activée sur la carte réseau qui sera utilisée pour recevoir et transmettre les paquets de la période d’enquête. La section ci-dessous illustre la définition de la carte réseau pour FW1 dans le fichier azuredeploy-nsg-udr.json, en fonction du scénario ci-dessus.
...
3. À partir de votre navigateur, accédez à **https://raw.githubusercontent.com/telmosampaio/azure-templates/master/IaaS-NSG-UDR/azuredeploy.parameters.json**, copiez le contenu du fichier json et coller dans un nouveau fichier sur votre ordinateur. Dans ce scénario, vous souhaitez copie les valeurs ci-dessous dans un fichier nommé **c:\udr\azuredeploy.parameters.json**.
...
4. Exécutez l’applet de commande **créer de groupe azure** pour déployer le nouveau VNet en utilisant les fichiers de modèle et de paramètres vous avez téléchargé et modifier les propriétés. La liste est indiquée après que la sortie explique les paramètres utilisés.
...
5. Exécutez la commande **azure groupe Afficher** pour afficher les ressources créées dans le nouveau groupe de ressources. 
...
>[AZURE.TIP] Si vous ne voyez pas toutes les ressources, exécutez le `azure group deployment show` commande pour vérifier l’état de mise en service du déploiement est *réussi*.