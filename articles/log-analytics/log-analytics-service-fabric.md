...
Cet article décrit comment utiliser la solution de Service Fabric dans Analytique du journal afin d’identifier et de résoudre les problèmes sur votre cluster Service Fabric, effectuent en obtenant la visibilité dans la façon dont vos nœuds de Service Fabric et comment vos applications et micro-services sont en cours d’exécution.

La solution de Service Fabric utilise des données de Diagnostics d’Azure à partir de vos ordinateurs virtuels Fabric de Service, par la collecte des données de vos tables Azure WAD. Analytique de journal lit ensuite les événements de framework Service Fabric, y compris **Les événements de Service fiables**, acteur, **Les événements opérationnels**et **Événements** **événements personnalisés ETW**. Le tableau de bord de solution Service Fabric montre vous problèmes et événements pertinents dans votre environnement Fabric du Service.

## <a name="installing-and-configuring-the-solution"></a>L’installation et la configuration de la solution

Suivez ces trois étapes pour installer et configurer la solution :

1. Assurez-vous que l’espace de travail OMS que vous utilisez est associé à l’abonnement Azure même qui vous permet de créer toutes les ressources de cluster, y compris les comptes de stockage. Pour plus d’informations sur la création d’un espace de travail de l’OMS, consultez [mise en route de journal Analytique](log-analytics-get-started.md) .
2. Configurer OMS pour collecter et afficher les journaux de Service Fabric.
3. Activer la solution de Service Fabric dans votre espace de travail.

## <a name="configure-oms-to-collect-and-view-service-fabric-logs"></a>Configurer OMS pour collecter et afficher les journaux de Service Fabric
Dans cette section, vous allez apprendre à configurer OMS pour récupérer les journaux de Service Fabric. Les journaux permettent de vous affichez, analysez et résoudre les problèmes de votre cluster ou dans les applications et les services qui s’exécutent dans un cluster, l’utilisation du portail de l’OMS.

>[AZURE.NOTE] L’extension de Diagnostics d’Azure doit être configurée pour télécharger les journaux vers les tables de stockage qui correspondent à ce que recherchera OMS. Pour plus d’informations sur la collecte des journaux, reportez-vous à la section [comment collecter les journaux de diagnostic d’Azure](../service-fabric/service-fabric-diagnostics-how-to-setup-wad.md) . Les exemples de paramètres de configuration dans cet article montrent que quels sont les noms des tables de stockage doit vous. Une fois le diagnostic est configurée sur le cluster et transfère les journaux vers un compte de stockage, l’étape suivante consiste à configurer les OMS pour la récupération de ces journaux.

Assurez-vous que vous mettez à jour la section **EtwEventSourceProviderConfiguration** dans le fichier **template.json** pour ajouter des entrées pour la nouvelle EventSources avant d’appliquer la mise à jour de configuration en exécutant **deploy.ps1**. La table pour le chargement est identique (ETWEventTable). Pour le moment, OMS peut uniquement lire les événements ETW d’application à partir de cette table. Toutefois, prise en charge des tables personnalisées ETW est en cours de développement.

Les outils suivants sont utilisés pour effectuer certaines opérations dans cette section :

-   PowerShell Azure
-   [Suite de gestion des opérations](http://www.microsoft.com/oms)

### <a name="configure-an-oms-workspace-to-show-the-cluster-logs"></a>Configurer un espace de travail de l’OMS pour afficher les journaux de cluster

Après avoir créé un espace de travail OMS, comme décrit ci-dessus, l’étape suivante consiste à configurer l’espace de travail pour extraire les journaux à partir des tables de stockage Azure où ils sont en cours de téléchargement à partir du cluster par l’extension des tests de diagnostic. Pour ce faire, exécutez le script PowerShell suivant :
...
Une fois que vous avez configuré l’espace de travail OMS pour lire les tables Azure dans votre compte de stockage, le portail Azure, connectez-vous et sélectionnez l’espace de travail OMS de **Toutes les ressources**. Une fois sélectionné, vous devriez voir le nombre de stockage connectés à cet espace de travail OMS de journaux de compte. Sélectionnez le carreau de **journaux de compte de stockage** et vérifiez dans la liste des journaux de compte de stockage que votre compte de stockage est connecté à cet espace de travail d’OMS :

![Journaux de compte de stockage](./media/log-analytics-service-fabric/sf1.png)

## <a name="enable-the-service-fabric-solution"></a>Activer la solution de Service Fabric
Utilisez le script suivant pour ajouter la solution à votre espace de travail de l’OMS. Exécutez le script dans PowerShell, à l’aide de l’abonnement associé à l’espace de travail OMS que vous souhaitez activer la solution de Service Fabric dans Azure.
...
Après l’activation de la solution, la mosaïque de tissu de Service est ajoutée à votre page de vue d’ensemble de l’OMS, en vue de problèmes tels que les défaillances de runAsync et les annulations qui sont sont produites au cours des 24 dernières heures.

![Mosaïque de tissu de service](./media/log-analytics-service-fabric/sf2.png)

### <a name="view-service-fabric-events"></a>Afficher les événements de Service Fabric

Cliquez sur la **Service Fabric** mosaïque pour ouvrir le tableau de bord de Service Fabric. Le tableau de bord inclut les colonnes dans le tableau suivant. Chaque colonne répertorie les événements de dix supérieur en nombre correspondant aux critères de cette colonne pour la plage de temps spécifiée. Vous pouvez exécuter une recherche de journal qui fournit la liste complète en cliquant sur **Afficher tout** en bas, à droite de chaque colonne, ou en cliquant sur l’en-tête de colonne.
...
>[AZURE.NOTE] Vous pouvez modifier la portée de ces événements dans la solution de Service Fabric en cliquant sur les **données basées sur les 7 derniers jours** en haut du tableau de bord. Vous pouvez également afficher les événements générés dans les 7 derniers jours, 1 jour ou 6 heures. Ou bien, vous pouvez sélectionner **personnalisée** pour spécifier une plage de dates personnalisée.


## <a name="troubleshoot-your-service-fabric-and-oms-configuration"></a>Résoudre les problèmes de configuration de votre Service de Fabric et OMS

Si vous avez besoin vérifier votre configuration d’OMS, car vous ne pouvez pas afficher les données d’événement de l’OMS, utilisez le script ci-dessous. Il lit votre configuration de diagnostics Service Fabric, vérifie les données écrites dans les tables, et vérifie que l’OMS est configuré pour lire à partir des tables.
