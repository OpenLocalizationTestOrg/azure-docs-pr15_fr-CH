<properties 
    pageTitle="Comment générer des miniatures à l’aide de Media Encoder Standard avec .NET" 
    description="Cette rubrique montre comment utiliser .NET pour coder une immobilisation et générer des miniatures à la fois à l’aide de Media Encoder Strandard." 
...
#<a name="how-to-generate-thumbnails-using-media-encoder-standard-with-net"></a>Comment générer des miniatures à l’aide de Media Encoder Standard avec .NET

Cette rubrique indique comment utiliser Media Services .NET SDK pour coder une immobilisation et générer des miniatures à l’aide de Media Encoder Standard. La rubrique définit les XML et JSON miniature paramètres prédéfinis que vous pouvez utiliser pour créer une tâche qui effectue le codage et la génère des miniatures en même temps. [Ce](https://msdn.microsoft.com/library/mt269962.aspx) document contient des descriptions des éléments qui sont utilisés par ces paramètres prédéfinis.

Veillez à passer en revue la [Considérations relatives à la](media-services-dotnet-generate-thumbnail-with-mes.md#considerations) section.
...
- Charger le prédéfinies [XML](media-services-dotnet-generate-thumbnail-with-mes.md#xml) ou [JSON](media-services-dotnet-generate-thumbnail-with-mes.md#json) qui contiennent le codage prédéfinis ainsi que des informations nécessaires à la génération des miniatures. Vous pouvez enregistrer cette [JSON](media-services-dotnet-generate-thumbnail-with-mes.md#json) ou [XML](media-services-dotnet-generate-thumbnail-with-mes.md#xml) dans un fichier et utiliser le code suivant pour charger le fichier.
...
##<a id="json"></a>Préréglage JSON miniature
...
##<a id="xml"></a>Préréglage XML miniature
