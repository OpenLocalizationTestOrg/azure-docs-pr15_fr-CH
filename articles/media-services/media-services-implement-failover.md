<properties 
    pageTitle="Mise en œuvre de la diffusion en continu de scénario de basculement | Microsoft Azure" 
    description="Cette rubrique indique comment implémenter le scénario de diffusion en continu de basculement." 
...
#<a name="implementing-failover-streaming-scenario"></a>Mise en œuvre de la diffusion en continu de scénario de basculement

Cette procédure pas à pas montre comment copier le contenu (BLOB) d’une immobilisation vers une autre afin de gérer la redondance pour la diffusion en continu à la demande. Ce scénario est utile pour les clients qui souhaitent configurer leur CDN pour basculer entre les deux centres de données en cas d’arrêt dans un de nos centres de données.
Cette procédure pas à pas utilise Microsoft Azure Media Services SDK et API REST de Microsoft Azure Media Services SDK de stockage Azure pour illustrer les tâches suivantes.

1. Configurer un compte de Services de support dans le « Centre de données A ».
1. Télécharger un fichier mezzanine dans un élément de la source.
1. Codage de l’élément dans les fichiers de taux multi-bits MP4. 
1. Créer un localisateur SAS en lecture seule pour l’actif de la source d’avoir un accès en lecture au conteneur dans le compte de stockage associé à l’actif de la source.
1. Obtenir le nom du conteneur de l’actif de la source à partir du repère SAS en lecture seule est créé à l’étape précédente. Nous avons besoin de ces informations pour copier des objets BLOB entre des comptes de stockage (expliqués plus loin dans la rubrique.)
1. Créer un localisateur d’origine de l’actif qui a été créé par la tâche de codage. 

Puis, pour gérer le basculement :

1. Configurer un compte de Services de support dans « B de centre de données ».
1. Créer un élément vide de cible dans le compte de Services de support de la cible.
1. Créer un localisateur SAS d’écriture pour l’immobilisation cible vide de disposer d’un accès en écriture au conteneur dans le compte de stockage associé à la ressource cible de la cible.
1. Utilisez le Kit de développement de stockage Azure pour copier des objets BLOB (fichiers actifs) entre le compte de stockage source dans « Centre de données A » et le compte de stockage cible dans « B de centre de données » (ces comptes de stockage sont associés à des actifs de l’intérêt.)
1. Associer des objets BLOB (fichiers de ressources) qui ont été copiés vers le conteneur d’objet blob cible avec la ressource cible. 
1. Créer un localisateur d’origine de l’immobilisation dans « B de centre de données » et spécifier le localisateur Id qui a été généré pour l’actif dans le « Centre de données A ». 
1. Cela vous donne l’URL en continu où les chemins d’accès relatifs de l’URL sont identiques (uniquement la base de l’URL sont différentes). 
 
Ensuite, pour gérer les interruptions de service, vous pouvez créer un CDN sur ces repères d’origine. 

Les considérations suivantes s’appliquent :

- La version actuelle de Media Services SDK ne gère pas la création d’un localisateur avec un identificateur de recherche spécifié. Pour réaliser cette tâche, nous allons utiliser l’API REST de Services média.
- La version actuelle de Media Services SDK ne gère pas la génération par programme les informations IAssetFile seraient d’associer un actif avec les fichiers de ressources. Pour ce faire, nous allons utiliser l’API REST de CreateFileInfos Media Services. 
- Des ressources de stockage crypté (AssetCreationOptions.StorageEncrypted) ne sont pas pris en charge pour la réplication (dans la mesure où la clé de cryptage sera différente dans les deux comptes de Media Services). 
- Si vous souhaitez tirer parti de l’emballage dynamique, vous devez d’abord obtenir au moins une demande réservée d’unités de transmission en continu. Pour plus d’informations, reportez-vous à la section [Dynamiquement les ressources d’emballage](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Envisagez d’utiliser [l’Outil du réplicateur](http://replicator.codeplex.com/) Media Services comme alternative à l’implémentation d’un scénario de diffusion en continu de basculement manuellement. Cet outil vous permet de répliquer des actifs entre les deux comptes de Media Services.

##<a name="prerequisites"></a>Conditions préalables
 
- Deux comptes de Services de support dans un abonnement Azure nouvel ou existant. Apprendre [à créer un compte de Services de support](media-services-portal-create-account.md).
- Systèmes d’exploitation : Windows 7, Windows 2008 R2 ou Windows 8.
- Le.NET Framework 4.5 ou.NET Framework 4.
...
1. Utilisez Visual Studio pour créer une nouvelle solution qui contient le projet d’Application de Console C#. Entrez HandleRedundancyForOnDemandStreaming pour le nom et puis cliquez sur OK.
1. Créez le dossier SupportFiles au même niveau que le fichier de projet HandleRedundancyForOnDemandStreaming.csproj. Sous le dossier SupportFiles créer les dossiers de débutFichiers et MP4Files. Copier un fichier .mp4 dans le dossier MP4Files (dans cet exemple, que le fichier BigBuckBunny.mp4 est utilisé). 
...
1. Ajouter d’autres références qui sont nécessaires pour ce projet : System.Web System.Configuration et System.Runtime.Serialization.
...
1. Remplacez la définition de méthode principal par défaut par celle qui suit :
...
Vous pouvez désormais utiliser le Gestionnaire de trafic pour router les demandes entre les deux centres de données et ainsi de basculement dans le cas des interruptions de service.
