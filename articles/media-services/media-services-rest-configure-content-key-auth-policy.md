<properties 
    pageTitle="Configurer la stratégie de d’autorisation clé de contenu à l’aide des API de Media Services reste | Microsoft Azure" 
    description="Découvrez comment configurer une stratégie d’autorisation pour une clé de contenu à l’aide des API de Media Services REST." 
...
#<a name="dynamic-encryption-configure-content-key-authorization-policy"></a>Cryptage dynamique : configurer la stratégie d’autorisation clé de contenu
[AZURE.INCLUDE [media-services-selector-content-key-auth-policy](../../includes/media-services-selector-content-key-auth-policy.md)]


##<a name="overview"></a>Vue d’ensemble

Microsoft Azure Media Services vous permet de fournir le contenu crypté (dynamiquement) avec la norme AES (Advanced Encryption) (à l’aide de clés de cryptage à 128 bits) et PlayReady ou Widevine DRM. Media Services fournit également un service de distribution des clés et des licences PlayReady/Widevine aux clients autorisés.

Si vous souhaitez que pour les Services de support crypter une immobilisation, vous devez associer une clé de cryptage (**CommonEncryption** ou **EnvelopeEncryption**) de l’actif (comme décrit [ici](media-services-rest-create-contentkey.md)) et également de configurer les stratégies d’autorisation pour la clé (comme décrit dans cet article).


Lorsqu’un flux est demandé par un lecteur, Media Services utilise la clé spécifiée pour dynamiquement chiffrer votre contenu en utilisant le chiffrement AES ou PlayReady. Pour déchiffrer le flux de données, le lecteur demande la clé à partir du service de distribution de clés. Pour décider ou non de l’utilisateur est autorisé à obtenir la clé, le service évalue les stratégies d’autorisation que vous avez spécifiée pour la clé.

Media Services prend en charge plusieurs méthodes d’authentification des utilisateurs qui effectuent des demandes de clés. La stratégie d’autorisation de clés contenu peut avoir une ou plusieurs restrictions d’autorisation : restriction **d’Ouvrir** ou de **jeton** . La stratégie de jeton restreint doit être accompagnée d’un jeton émis par un Service (jeton de sécurité). Media Services prend en charge les jetons dans les formats de **Jetons de Web Simple** ([SWT](https://msdn.microsoft.com/library/gg185950.aspx#BKMK_2)) et **JSON Web jeton **(JWT).
...
####<a id="ContentKeyAuthorizationPolicies"></a>Créer des ContentKeyAuthorizationPolicies
...
####<a id="ContentKeyAuthorizationPolicyOptions"></a>Créer des ContentKeyAuthorizationPolicyOptions
...
####<a id="LinkContentKeyAuthorizationPoliciesWithOptions"></a>ContentKeyAuthorizationPolicies lien avec Options
...
####<a id="AddAuthorizationPolicyToKey"></a>Ajouter la stratégie d’autorisation pour la clé de contenu
...
###<a name="create-contentkeyauthorizationpolicies"></a>Créer des ContentKeyAuthorizationPolicies

Créer la « stratégie de Restriction « jeton comme indiqué [ici](#ContentKeyAuthorizationPolicies).


###<a name="create-contentkeyauthorizationpolicyoptions"></a>Créer des ContentKeyAuthorizationPolicyOptions
...
####<a name="link-contentkeyauthorizationpolicies-with-options"></a>ContentKeyAuthorizationPolicies lien avec Options

Lien ContentKeyAuthorizationPolicies avec des Options comme indiqué [ici](#ContentKeyAuthorizationPolicies).

####<a name="add-authorization-policy-to-the-content-key"></a>Ajouter la stratégie d’autorisation pour la clé de contenu

Ajouter AuthorizationPolicy à la ContentKey, comme illustré [ici](#AddAuthorizationPolicyToKey).
...
Lorsque vous protégez votre contenu avec PlayReady, l’une des choses que vous devez spécifier dans votre stratégie d’autorisation est une chaîne XML qui définit le [modèle de licence PlayReady](https://msdn.microsoft.com/library/azure/dn783459.aspx). 
...
####<a id="ContentKeyAuthorizationPolicies2"></a>Créer des ContentKeyAuthorizationPolicies
...
Créez ContentKeyAuthorizationPolicies comme illustré [ici](#ContentKeyAuthorizationPolicies2).
...
Maintenant que vous avez configuré la stratégie d’autorisation de la clé de contenu, consultez la rubrique [comment configurer la stratégie de livraison des actifs](media-services-rest-configure-asset-delivery-policy.md) .
