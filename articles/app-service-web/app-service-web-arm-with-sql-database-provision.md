<properties 
    ...
    description="Un modèle de gestionnaire de ressources Azure permet de déployer une application web qui inclut une base de données SQL." 
...
Pour plus d’informations sur le déploiement des applications, voir [déploiement d’une application complexe prévisible dans Azure](app-service-deploy-complex-application-predictably.md).

Pour le modèle complet, consultez [modèle de Web App avec SQL de base de données](https://github.com/Azure/azure-quickstart-templates/blob/master/201-web-app-sql-database/azuredeploy.json).
...
- une application web
- Serveur de base de données de SQL
- Base de données SQL
...
[![Déployer vers Azure](./media/app-service-web-arm-with-sql-database-provision/deploybutton.png)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2F201-web-app-sql-database%2Fazuredeploy.json)
...
Le nom de la nouvelle base de données à créer.
...
Le classement de la base de données à utiliser pour régissant l’utilisation correcte des caractères.
...
Le nom correspondant pour le niveau de performance pour édition. 
...
Ce modèle inclut des variables qui construisent des noms utilisés dans le modèle. Les valeurs de variable utilisent la fonction **uniqueString** pour générer un nom à partir de l’id de groupe de ressources.
...
### <a name="sql-server-and-database"></a>Base de données et SQL Server

Crée un nouveau SQL Server et base de données. Le nom du serveur est spécifié dans le paramètre **nom du serveur** et de l’emplacement spécifié dans le paramètre **serverLocation** . Lorsque vous créez le nouveau serveur, vous devez fournir un nom d’utilisateur et le mot de passe pour l’administrateur du serveur de base de données. 
...
### <a name="alert-rules-for-status-codes-403-and-500s-high-cpu-and-http-queue-length"></a>Règles d’alerte pour les codes d’état 403 et 500's, élevée du processeur et longueur de file d’attente HTTP 
...
### <a name="app-insights"></a>Aperçu de l’application
